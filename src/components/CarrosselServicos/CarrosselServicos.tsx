...
export function Carrossel() {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      >
        <div className="flex w-full h-full select-none cursor-grab active:cursor-grabbing">
          {slides.map((slide) => (
            <div
              key={slide.id}
            ...
            >
              <img 
                src={slide.img} 
                alt={slide.alt} 
                className="object-container w-full h-full pointer-events-none"
              />
            </div>
          ))}
        </div>
      ...
      <button
        ...
        className="absolute left-1 top-1/2 -translate-y-1/2 p-4 transition text-[var(--branco)] hover:opacity-80 z-10"
      >
        <GrPrevious size={30} />
      </button>
      <button
        ...
        className="absolute right-1 top-1/2 -translate-y-1/2 p-4 transition text-[var(--branco)] hover:opacity-80 z-10"
      >
        <GrNext size={30} />
      </button>
    </div>
