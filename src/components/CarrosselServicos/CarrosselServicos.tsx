...
export function Carrossel() {
  ...
  return (
    <div className="relative h-full">
      <div className="overflow-hidden h-full" ref={emblaRef}>
        <div className="flex h-full">
          {slides.map((slide) => (
            <div
              key={slide.id}
              className={`h-full flex-[0_0_40%] ${slide.tipo === "vertical" ? "flex-[0_0_55%] md:flex-[0_0_44%] lg:flex-[0_0_auto]" : "flex-[0_0_120%] md:flex-[0_0_95%] lg:flex-[0_0_auto]" }`}
            >
              <img
                src={slide.img}
                alt={slide.alt}
                className="h-full w-full lg:px-1.5 object-contain"
              />
            </div>
          ))}
        </div>
      ...
      <button
        onClick={scrollPrev}
        className="absolute left-6 top-1/2 -translate-y-1/2 z-10 text-amber-200"
      >
        <GrPrevious size={45} />
      </button>
      ...
      <button
        onClick={scrollNext}
        className="absolute right-6 top-1/2 -translate-y-1/2 z-10 text-amber-200"
      >
        <GrNext size={45} />
      </button>
    </div>
...
export function Carrossel() {
  ...
  return (
    <div
      ref={emblaRef}
      className="overflow-hidden flex flex-col justify-center items-center w-full h-full z-20 "
    >
      <div className="flex w-[100%] md:w-[90%] lg:w-[100%] h-full select-none cursor-grab">
        {slides.map((slide) => (
          <div
            key={slide.id}
      ...
      <button
        onClick={scrollPrev}
        className="absolute left-1  p-4 transition text-[var(--branco)]"
      >
        <GrPrevious size={30} />
      </button>
      <button
        onClick={scrollNext}
        className="absolute right-1 p-4 transition text-[var(--branco)]"
      >
        <GrNext size={30} />
      </button>
    </div>
